...
app.post('/api/save-an-snapshot', async (req, res) => {
  ...
  pol: pol || null,
  pod: pod || null,
  place_of_receipt: place_of_receipt ?? null,
...
app.get('/api/an-input', async (req, res) => {
  try {
    ...
    const resolved = {
      ...
      place_of_receipt: snapshot?.place_of_receipt ?? '',
      place_of_delivery: snapshot?.place_of_delivery ?? '',
...
app.get('/api/admin/master-data', async (req, res) => {
  try {
    ...
    const truckers = (partners || []).filter(p =>
      p.partner_type === 'TRUCK' ||
      p.partner_type === 'Trucker' ||
      p.partner_type === '配送業者'
    );
